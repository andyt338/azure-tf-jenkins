...
resource "azurerm_storage_account" "redapt86" {
    name = "redapt86"
    resource_group_name = "${azurerm_resource_group.helloterraform.name}"
    location = "westus"
...
resource "azurerm_storage_container" "redapt86storagecontainer" {
    name = "vhd"
    resource_group_name = "${azurerm_resource_group.helloterraform.name}"
    storage_account_name = "${azurerm_storage_account.redapt86.name}"
    container_access_type = "private"
    depends_on = ["azurerm_storage_account.redapt86"]
}
...
resource "azurerm_virtual_machine" "helloterraformvm" {
    ...
    storage_os_disk {
        name = "myosdisk"
        vhd_uri = "${azurerm_storage_account.redapt86.primary_blob_endpoint}${azurerm_storage_container.redapt86storagecontainer.name}/myosdisk.vhd"
        caching = "ReadWrite"
